...
function socketHandler(io) {
    io.on("connection", (socket) => {
            ...
            if (existing && existing.socket.id !== socket.id) {
                ...
                if (existing.status === "busy" && existing.roomId) {
                    ...
                    if (room) {
                        ...
                        rooms.delete(roomId);
                        logger.info(`Room ${roomId} closed due to duplicate login for ${sessionId}`);
                    }
                }
        ...
        socket.on("joinQueue", ({
            ...
            if (!sessionId) return socket.emit("error", {
                code: "NOT_REGISTERED",
        ...
        socket.on("message", async ({
            ...
            if (!ensureRegistered()) return socket.emit("error", {
                code: "NOT_REGISTERED",
                message: "Register first"
            });
            if (!text ?.trim()) return;
    ...
    function leaveCurrentRoom(state, socket) {
        ...
        rooms.delete(roomId);
        logger.info(`Room ${roomId} closed. ${socket.sessionId} left; ${partner.sessionId} idle.`);
    }
    ...
    function tryMatchFromQueue() {
        ...
        for (const [mode, sids] of byMode.entries()) {
            ...
            while (sids.length >= 2) {
                ...
                queue = queue.filter((sid) => sid !== sa.sessionId && sid !== sb.sessionId);
                ...
                logger.info(`Room created ${roomId} for ${sa.sessionId} & ${sb.sessionId} | mode=${mode}`);
            }
        }
    }
